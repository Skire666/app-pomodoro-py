"""Collection of pomodoro session history entries."""
...
class PomodoroHistoryCollectionModel:
    """An ordered collection of `PomodoroHistoryEntryModel` instances."""

    def __init__(
        self, items: list[PomodoroHistoryEntryModel], created_at: datetime, modified_at: datetime
    ) -> None:
    ...
    def __iter__(self) -> Iterator[PomodoroHistoryEntryModel]:
    ...
    def __getitem__(self, index: int) -> PomodoroHistoryEntryModel:
        """Return the history entry stored at `index`."""
        return self._items[index]

    @property
    def items(self) -> tuple[PomodoroHistoryEntryModel, ...]:
        """All history entries currently held by this collection, in storage order."""
        return tuple(self._items)

    @items.setter
    def items(self, value: list[PomodoroHistoryEntryModel]) -> None:
    ...
    def create(self, item: PomodoroHistoryEntryModel) -> None:
        """Append a new history entry to the collection."""
        self._items.append(item)

    def read(self, id_history_entry: str) -> PomodoroHistoryEntryModel | None:
    ...
    def purge(self, max_entries: int) -> int:
        ...
        if len(self._items) <= max_entries:
            return 0
        ordered = sorted(self._items, key=lambda item: item.executed_at)
    ...
    def search(self, *, most_recent_first: bool = True) -> tuple[PomodoroHistoryEntryModel, ...]:
        """Return every entry sorted by execution date (spec §2.6 default sort).
        ...
        """
        return tuple(sorted(self._items, key=lambda item: item.executed_at, reverse=most_recent_first))
    ...
    def deserialize(cls, data: dict[str, Any]) -> Self:
        ...
        """
        return cls(
            items=[PomodoroHistoryEntryModel.from_dict(raw) for raw in data["items"]],
